        ...
        if (ObjectUtils.isNotEmpty(mysqlList)) {
            if (ObjectUtils.isNotEmpty(oracleList)) {
                ...
                while (iterator.hasNext()) {
                    SdHplStockOut mysql = iterator.next();
                    Iterator<SdHplStockOut> oracleIt = oracleList.iterator();
                    while (oracleIt.hasNext()) {
                        SdHplStockOut oracle = oracleIt.next();
                        if (mysql.getFormId().equals(oracle.getFormId())) {
                            if (!mysql.toString().equals(oracle.toString())) {
                                updateList.add(mysql);
                            }
                            iterator.remove();
                            oracleIt.remove();
                            break;
